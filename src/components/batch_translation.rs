...
fn TranslationResults(docs: ReadSignal<Vec<crate::services::batch_service::TranslatedDocument>>) -> impl IntoView {
    ...
    let download_selected = move |_| {
        let selected_docs: Vec<_> = docs_selection.get().into_iter()
        ...
        match service.create_compressed_archive(&selected_docs) {
            Ok(zip_data) => {
                ...
                anchor.set_href(&url);
                anchor.set_download(&format!("translated_docs_{}.tar.gz", 
                    js_sys::Date::new_0().get_time() as u64));
                anchor.click();
                
                web_sys::Url::revoke_object_url(&url).unwrap();
                web_sys::console::log_1(&format!("已下载 {} 个选中文档", selected_docs.len()).into());
            }
            Err(e) => {
        ...
        <div class="mb-6">
            ...
            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                ...
                <p class="text-green-700 dark:text-green-300 text-sm mb-4">
                    "选择要下载的文档，系统将按索引顺序打包为tar.gz文件。解压后会保持文件夹结构，文件名前有序号。"
                </p>
        ...
        <div class="space-y-3">
            <For
                each=move || {
                    ...
                    // 按文件夹分组
                    let mut folders: std::collections::HashMap<String, Vec<(usize, crate::services::batch_service::TranslatedDocument)>> = std::collections::HashMap::new();
                    for (index, doc) in docs.iter().enumerate() {
                        folders.entry(doc.folder_path.clone())
                            .or_insert_with(Vec::new)
                            .push((index, doc.clone()));
                    }
                    folders.into_iter().collect::<Vec<_>>()
                }
                key=|(folder, _)| folder.clone()
                children=move |(folder_path, docs_in_folder)| {
                    view! {
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                            <div class="bg-gray-50 dark:bg-gray-800 px-4 py-2 border-b border-gray-200 dark:border-gray-700">
                                <h4 class="text-sm font-medium text-gray-900 dark:text-white flex items-center gap-2">
                                    ...
                                    {if folder_path.is_empty() || folder_path == "documents" {
                                        "根目录".to_string()
                                    } else {
                                        folder_path.clone()
                                    }}
                                    <span class="text-xs text-gray-500 ml-1">
                                        "(" {docs_in_folder.len()} " 个文件)"
                                    </span>
                                </h4>
                            </div>
                            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                                <For
                                    each=move || docs_in_folder.clone()
