...
pub fn UrlInput(
    ...
    view! {
        <div>
            <label class="block text-sm font-medium mb-2 themed-text">
                "输入URL"
            </label>
            <div class="flex space-x-2">
                <input
                    type="url"
                    class="flex-1 px-4 py-2 border rounded-md focus:ring-2 focus:border-transparent themed-input"
                ...
                />
                <button
                    class="px-6 py-2 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors hover:opacity-90 themed-button-primary"
                    disabled=is_loading
                    on:click=on_submit
                >
