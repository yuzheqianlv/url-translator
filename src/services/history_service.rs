use crate::types::history::{
    BatchDocumentInfo, HistoryEntry, HistoryEntryType, HistoryFilter, HistorySortBy,
};
...
impl HistoryService {
        ...
        match format {
            ...
            ExportFormat::Csv => {
                let mut csv = String::from(
                    "ID,URL,Title,Source Language,Target Language,Created At,Word Count\n",
                );
                for entry in entries {
                    csv.push_str(&format!(
                        "{},{},{},{},{},{},{}\n",
    ...
    /// 下载批量翻译记录
    pub fn download_batch_translation(
        &self,
        entry_id: &str,
        selected_docs: Option<Vec<usize>>,
        ...
        match entry.entry_type {
            HistoryEntryType::BatchTranslation => {
                let batch_data = entry
                    .batch_data
                    .as_ref()
                    .ok_or("批量翻译数据缺失".to_string())?;

                // 确定要下载的文档
                let docs_to_download: Vec<&BatchDocumentInfo> =
                    if let Some(selected_indices) = selected_docs {
                        selected_indices
                            .iter()
            ...
            HistoryEntryType::SinglePage => Err("该记录是单页翻译，请使用单页下载功能".to_string()),
        }
    }
