...
use crate::services::history_service::{HistoryService, HistoryStatistics, ExportFormat};
            ...
            match history_service.search_entries(filter, sort) {
                Ok(filtered_entries) => {
                    ...
                    match history_service.get_statistics() {
                        ...
                        Err(e) => {
                            error_handler.handle_error(AppError::config(format!("获取统计信息失败: {}", e)));
                        }
                    }
                }
                Err(e) => {
                    error_handler.handle_error(AppError::config(format!("加载历史记录失败: {}", e)));
                }
            }
        ...
        move |_| {
            if let Some(entry) = add_trigger.get() {
                match history_service.add_entry(entry) {
                    ...
                    Err(e) => {
                        error_handler.handle_error(AppError::config(format!("添加历史记录失败: {}", e)));
                    }
                }
            }
        }
        ...
        move |_| {
            if let Some(id) = delete_trigger.get() {
                match history_service.delete_entry(&id) {
                    ...
                    Err(e) => {
                        error_handler.handle_error(AppError::config(format!("删除历史记录失败: {}", e)));
                    }
                }
            }
        }
        ...
        move |_| {
            if clear_trigger.get() {
                match history_service.clear_history() {
                    ...
                    Err(e) => {
                        error_handler.handle_error(AppError::config(format!("清空历史记录失败: {}", e)));
                    }
                }
                ...
                match history_service.export_history(format) {
                    Ok(content) => {
                        ...
                        if let Err(e) = create_and_download_file(&content, filename, "text/plain") {
                            error_handler.handle_error(AppError::config(format!("下载文件失败: {:?}", e)));
                        } else {
                            web_sys::console::log_1(&"历史记录导出成功".into());
                        }
                    }
                    Err(e) => {
                        error_handler.handle_error(AppError::config(format!("导出历史记录失败: {}", e)));
                    }
                }
            }
        }
...
fn create_and_download_file(content: &str, filename: &str, _mime_type: &str) -> Result<(), wasm_bindgen::JsValue> {
